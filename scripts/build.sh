...
base_chroot=epel-6-${arch}
mock_chroot=epel-6-varnish-${arch}

varnish_repo_id='varnish-4.1'
varnish_repo_name='Varnish Cache 4.1 for Enterprise Linux'
# shellcheck disable=SC2016
varnish_repo_baseurl='https://repo.varnish-cache.org/redhat/varnish-4.1/el6/$basearch'
...
download_source_files() {
  ...
  spectool --all -g "${topdir}"/SPECS/libvmod-header.spec -C "${topdir}"/SOURCES
}

create_varnish_repo_file() {
  varnish_repo_file=varnish-4.1.repo
  if [ ! -f $varnish_repo_file ]; then
    # NOTE: Although https://repo.varnish-cache.org/redhat/varnish-4.1.el6.rpm at https://www.varnish-cache.org/installation/redhat
...
\`\`\`"
  ...
  cat > /home/builder/.config/copr <<EOF
[copr-cli]
login = $COPR_LOGIN
